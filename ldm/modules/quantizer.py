...
class VectorQuantizer2_with_soft_dist(nn.Module):
    """
    Improved version over VectorQuantizer, can be used as a drop-in replacement. Mostly
    avoids costly matrix multiplications and allows for post-hoc remapping of indices.
    """
    ...
    def __init__(self, n_e, e_dim, beta, remap=None, unknown_index="random",
        ...
        self.sane_index_shape = sane_index_shape
        self.mem_dict = mem_dict if mem_dict is not None else {}
    ...
    def forward(self, z, temp=None, rescale_logits=False, return_logits=False, epoch=None, max_epoch=None):
        assert temp is None or temp==1.0, "Only for interface compatible with Gumbel"
        assert rescale_logits==False, "Only for interface compatible with Gumbel"
        assert return_logits==False, "Only for interface compatible with Gumbel"
        # reshape z -> (batch, height, width, channel) and flatten
        ...
        min_encoding_indices = torch.argmin(d, dim=1)
        encoding_inds = min_encoding_indices.unsqueeze(1)  # [BHW]
        # Convert to one-hot encodings
        device = z.device
        encoding_one_hot = torch.zeros(encoding_inds.size(0), self.n_e, device=device)
        ...
        # ---------- New losses for every embeding -----------
        if epoch is None or max_epoch is None:
            tau_init = float(self.mem_dict.get('tau', 1.0))
            # 2) 读 decay_dict；提供默认值，并在 epoch/max_epoch 缺失时回退
            decay_defaults = {
                "method": "exp",
                "epoch": 0 if epoch is None else int(epoch),
                "max_epoch": 1 if max_epoch is None else int(max_epoch),
                "initial_p": tau_init,
                "warmup_flag": False,
                "warmup_epochs": 0,
                "warmup_type": "linear",
            ...
            decay_dict = dict(decay_defaults)
            user_decay = self.mem_dict.get('decay_dict', {})
